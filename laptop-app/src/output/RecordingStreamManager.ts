...
export class RecordingStreamManager {
  ...
  private handleHeadlessOutput(sessionId: string, data: string): void {
    ...
    const isComplete = text.includes('[COMMAND_COMPLETE]');
    ...
    const state = this.getSessionState(sessionId);
    if (state.lastHeadlessDelta === cleanText && !isComplete) {
    ...
    console.log(`📤 [${sessionId}] Broadcasting recording output: fullText=${state.headlessFullText.length} chars, delta=${cleanText.length} chars`);
