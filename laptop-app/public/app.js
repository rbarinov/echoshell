...
const TERMINAL_TYPE_LABELS = {
    ...
    cursor_cli: '🤖 Cursor Agent (headless)',
...
function openSession(sessionId) {
    ...
    // Always initialize xterm.js terminal for all session types
    // For headless terminals, it will show read-only history
    initTerminal();
...
function initTerminal() {
    ...
    // Handle user input
    term.onData((data) => {
        // Send input via WebSocket for all terminal types
...
async function loadHistoryIntoTerminal(sessionId) {
    ...
    try {
        ...
        if (response.ok && data.history && term) {
            ...
            // For headless terminals, this will show all commands and output
...
function connectWebSocket(sessionId) {
    ...
    ws.onmessage = (event) => {
        ...
        if (data.type === 'output' && data.data && term) {
            ...
            // For headless terminals, this shows all commands and responses in real-time
