...
class SecureKeyStore {
    ...
    private let sttKeyAccount = "ephemeral_stt_key"
    private let ttsKeyAccount = "ephemeral_tts_key"
    ...
    func save(_ keys: KeyResponse.Keys, expiresAt: Date) {
        // Save STT key
        saveToKeychain(account: sttKeyAccount, value: keys.stt)
        
        // Save TTS key
        saveToKeychain(account: ttsKeyAccount, value: keys.tts)
    ...
    func load() -> KeyResponse.Keys? {
        guard let sttKey = loadFromKeychain(account: sttKeyAccount),
              let ttsKey = loadFromKeychain(account: ttsKeyAccount) else {
            return nil
        }
        
        return KeyResponse.Keys(stt: sttKey, tts: ttsKey)
    }
    ...
    func clear() {
        deleteFromKeychain(account: sttKeyAccount)
        deleteFromKeychain(account: ttsKeyAccount)
