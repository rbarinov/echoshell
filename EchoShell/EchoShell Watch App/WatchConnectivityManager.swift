        ...
        DispatchQueue.main.async {
            ...
            if let keysDict = context["ephemeralKeys"] as? [String: Any],
               ...
               let keys = try? JSONDecoder().decode(KeyResponse.Keys.self, from: keysData) {
                ...
                // Set apiKey from ephemeral keys for backward compatibility (use TTS key)
                self.settingsManager.apiKey = keys.tts
            ...
            if let endpointsDict = context["providerEndpoints"] as? [String: Any],
               let endpointsData = try? JSONSerialization.data(withJSONObject: endpointsDict),
               let endpoints = try? JSONDecoder().decode(KeyResponse.Endpoints.self, from: endpointsData) {
                self.settingsManager.providerEndpoints = endpoints
                print("✅ Watch: Updated provider endpoints")
            }
            
            // Update provider config
            if let configDict = context["providerConfig"] as? [String: Any],
               let configData = try? JSONSerialization.data(withJSONObject: configDict),
               let config = try? JSONDecoder().decode(KeyResponse.Config.self, from: configData) {
                self.settingsManager.providerConfig = config
                print("✅ Watch: Updated provider config")
            }
