...
**tmux Configuration (prevents interference):**
```typescript
// laptop-app/src/terminal/TerminalManager.ts
set-option -t session mouse off     // Don't intercept touch events
set-option -t session status off    // No status bar
set-option -t session history-limit 50000  // 50k lines of history
```
...
- ✅ Works with tmux mirroring active
...
2. Kills PTY process
3. Kills tmux session (if exists)
